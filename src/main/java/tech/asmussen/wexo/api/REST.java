...
public class REST {
	...
	public void fetch(int from, int to) {
		...
		try (Response response = client.newCall(request).execute()) {
			...
			for (int i = 0; i < entries.size(); i++) {
				...
				if (entry == null || activeCache.contains(entry)) continue;
	...
	private Entry defineEntry(JsonObject entry) {
		...
		if (thumbnails == null || thumbnails.isJsonNull() || thumbnails.size() == 0) return null;
		...
		for (String key : thumbnails.keySet()) {
			...
			if (url.contains("po") || url.contains("Poster")) covers.put(url, List.of(width, height));
			
			else if (url.contains("bd")) backdrops.put(url, List.of(width, height));
		}
		
		// Handle the genres.
		JsonArray tags = entry.getAsJsonArray("plprogram$tags");
		
		if (tags == null || tags.isJsonNull() || tags.size() == 0) return null;
		
		for (int i = 0; i < tags.size(); i++) {
			
			JsonObject tag = tags.get(i).getAsJsonObject();
			
			if (!tag.get("plprogram$scheme").getAsString().equalsIgnoreCase("genre")) continue;
		...
		if (credits == null || credits.isJsonNull() || credits.size() == 0) return null;
		...
		if (media == null || media.isJsonNull() || media.size() == 0) return null;
		...
		for (int i = 0; i < media.size(); i++) {
			...
			if (rawUrl == null || rawUrl.isJsonNull()) continue;
	...
	private Entry defineCachedEntry(JsonObject entry) {
		...
		if (coversObject == null || coversObject.isJsonNull() || coversObject.size() == 0) return null;
		
		if (backdropsObject == null || backdropsObject.isJsonNull() || backdropsObject.size() == 0) return null;
		...
		if (genresArray == null || genresArray.isJsonNull() || genresArray.size() == 0) return null;
		...
		if (actorsArray == null || actorsArray.isJsonNull() || actorsArray.size() == 0) return null;
		
		if (directorsArray == null || directorsArray.isJsonNull() || directorsArray.size() == 0) return null;
		...
		if (trailersArray == null || trailersArray.isJsonNull() || trailersArray.size() == 0) return null;
